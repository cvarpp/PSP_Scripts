...
# 1) Make sure $outDir exists
unless ( -d $outDir ) {
    mkdir $outDir
      or die "Cannot create output directory '$outDir': $!";
}

# 2) Make sure summaryOut exists
unless ( -d $summaryOut ) {
    mkdir $summaryOut
      or die "Cannot create summary directory '$summaryOut': $!";
}

# 3) Now create the passes directory
my $pass_dir = "$summaryOut/passedVariants_vcf";
unless ( -d $pass_dir ) {
    mkdir $pass_dir
      or die "Cannot create passed‐variants directory '$pass_dir': $!";
}
...
warn "Writing final VCF to: $filterVCF\n";
#Filter variants
my %pass_vcf;
...
foreach my $chr(@list){
	foreach my $pos(sort {$a <=> $b} keys %{$finalVCF{$chr}}){
		...
		if ( update_counter("1_duo_alignment", $duo_flag, $finalVCF{$chr}{$pos}) ) {
		next;
        }				
		#next if $duo_flag == 1;
		...
		# Update counter and check: if the variant fails, skip to the next.
        if ( update_counter("2_cluster_filter", $cluster_flag, $finalVCF{$chr}{$pos}) ) {
            next;
		}
		...
		# 	$edits_flag = 1;			
		# }
		# if ( update_counter("3_edits_filter", $edits_flag, $finalVCF{$chr}{$pos}) ) {
        #     next;
        # }
		...
		if ( update_counter("4_igg_filter", $igg_flag, $finalVCF{$chr}{$pos} ) ) {
			next;
		}
		next if $igg_flag == 1;
		...
		if ( update_counter("5_hla_filter", $hla_flag, $finalVCF{$chr}{$pos}) ) {
			next;
		}
		#next if $hla_flag == 1;
		...
		#next if $pon_flag == 1;
		if ( update_counter("6_pon_filter", $pon_flag, $finalVCF{$chr}{$pos} ) ) {
			next;
		}
		...
		if ( update_counter("7_length_flag", $length_flag, $finalVCF{$chr}{$pos} ) ) {
			next;
		}
		...
		if ( update_counter("8_normal_filter", $normal_flag, $finalVCF{$chr}{$pos} ) ){
			next;
		}
		...
		if ( update_counter("9_pon", $pon_flag, $finalVCF{$chr}{$pos} ) ){
			next;
		}
		#next if $pon_flag == 1;
		...
		if ( update_counter("10_tandem_filter", $tandem_flag, $finalVCF{$chr}{$pos} ) ){
			next;
		}
		#next if $tandem_flag == 1;
				
		#multiallelic
		#warn "DEBUG: at multiallelic for $chr:$pos ALT=", $line[4], "\n";
		my $multiallelic_flag = 0;			
		{#Split alleles by a comma, which should be in col 4, into an array
			my @alts = split /,/, $line[4]; 
			#Checks if the array is longer than 1 
			$multiallelic_flag = @alts > 1 ? 1 : 0;

			$multiallelic_flag = 1;			
		}
		if ( update_counter("11_multiallelic_filter", $multiallelic_flag, $finalVCF{$chr}{$pos} ) ){
			next;
		}
		...
		# 	#	#Total Alternative reads threshold filters
		# 	#	# Variant has to be present on the reverse and forward read only appear twice??
		# 	#	$mpileup_flag = 1 if ($forward_alt+$reverse_alt) <= 2;	
		# 	#	# If the forward/reverse reads with alterations is less than 10% of total reads, the are filtered out
		...
		if ( update_counter("12_samtools_mpileup_filter", $mpileup_flag, $finalVCF{$chr}{$pos} ) ){
			next;
		}
		...
		if ( update_counter("13_bcftool_filter", $bcf_flag, $finalVCF{$chr}{$pos} ) ){
			next;
		}
		...
		if ( update_counter("14_psuedo_filter", $pseudo_flag, $finalVCF{$chr}{$pos} ) ){
			next;
		}
		...
		# 	$pos_flag = 1;
		# }
		# if ( update_counter("15_position_filter", $pos_flag, $finalVCF{$chr}{$chr} ) ){
		# 	next;
		# }
		#next if $pos_flag ==1;
		...
		# 	$TLOD_flag = 1;					
		# }
		#if ( update_counter("16_TLOD_filter", $TLOD_flag, $finalVCF{$chr}{$pos} ) ){
		#	next;
		#}
		#next if $TLOD_flag == 1;
		...
		if ( update_counter("17_Mapping_quality_filter", $MMQ_flag, $finalVCF{$chr}{$pos} ) ){
			next;
		}
		#next if $MMQ_flag ==1;
		...
		# if ( update_counter("18_Tumor_cutoff", $tumor_flag, $finalVCF{$chr}{$pos} ) ){
		# 	next;
		# }
		...
		# 	$high_flag = 1;
		# }
		# if ( update_counter("19_high_mutation_freq", $high_flag, $finalVCF{$chr}{$pos} ) ){
		# 	next;
		# }
		#next if $high_flag == 1;
		...
		if ( update_counter("20_signle_nt_check", $Mono_flag, $finalVCF{$chr}{$pos} ) ){
			next;
		}				
		#next if $Mono_flag ==1;				
		...
		if ( update_counter("21_seq_quality_filter", $sequencing_quality_flag, $finalVCF{$chr}{$pos} ) ){
			next;
		}
		...
		if ( update_counter("22_coverage_filter", $coverage_flag, $finalVCF{$chr}{$pos} ) ){
			next;
		}				
...
# Summary of filtering of all steps in Filter Variants 

my $summary_log = "$summaryOut/filtering_summary.log";	
open(my $summary_fh, ">", $summary_log) or die "Cannot open filtering_summary.log for writing: $!\n";
#This will sort the keys numerically (1_duo_filter, 2.., ) to make the log more clear to read
foreach my $filter (sort { ($a =~ /^(\d+)_/)[0] <=> ($b =~ /^(\d+)_/)[0] } #This gets uses regex to get the identify the  numbers and sorts them in numerical order
                    keys %filter_counters) { #This access the keys created by running the filters that have subroutine update_counter (which should be all.)
    my $total  = $filter_counters{$filter}{total}  // 0;
    my $failed = $filter_counters{$filter}{failed} // 0;
    my $passed = $filter_counters{$filter}{passed} // 0;

    #next unless $failed > 0;    # if you only want filters with failures

    print $summary_fh "Filter: $filter\n";
    print $summary_fh "  Variants tested: $total\n";
    print $summary_fh "  Variants failed: $failed\n";
    print $summary_fh "  Variants passed: $passed\n\n";
}
...
#Counter to determien variants filtered during each step 
sub update_counter {
    my ($filter_name, $failed, $variant_line) = @_;

    # total++
    $filter_counters{$filter_name}{total} //= 0;
    $filter_counters{$filter_name}{total}++;

    $variant_line //= "";

    if ($failed) {
        # failed++
        $filter_counters{$filter_name}{failed} //= 0;
        $filter_counters{$filter_name}{failed}++;

        # log failures
        my $fail_log = "$summaryOut/${filter_name}_filter.log";
        open my $fh, '>>', $fail_log or die $!;
        print $fh "$variant_line\n";
        close $fh;
    }
    else {
        # passed++
        $filter_counters{$filter_name}{passed} //= 0;
        $filter_counters{$filter_name}{passed}++;

        # log passes
        my $pass_log = "$summaryOut/passedVariants_vcf/${filter_name}_passed.vcf";
        open my $fh, '>>', $pass_log or die $!;
        print $fh "$variant_line\n";
        close $fh;
    }
    return $failed;
