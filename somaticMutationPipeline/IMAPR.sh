...
# Get directory of where shell script is
BASEDIR=$(dirname "$0")
...
#input_dev_files, finalVCF, firstVCF, bed_file, bam
fnvcf=$(grep "^fnvcf" $input_file | awk -F '\t' '{print $2}' | tr -d '\n')
fivcf=$(grep "^fivcf" $input_file | awk -F '\t' '{print $2}' | tr -d '\n')
bedf=$(grep "^bedf" $input_file | awk -F '\t' '{print $2}' | tr -d '\n')
bamf=$(grep "^bamf" $input_file | awk -F '\t' '{print $2}' | tr -d '\n')

#SUMMARY FILE OUTPUT
summaryOut=$(grep "^summaryOut" $input_file | awk -F '\t' '{print $2}' | tr -d '\n')

#input files for stacking modle module
flvcf=$(grep "^flvcf" $input_file | awk -F '\t' '{print $2}' | tr -d '\n')
mout=$(grep "^mout" $input_file | awk -F '\t' '{print $2}' | tr -d '\n')
bout=$(grep "^bout" $input_file | awk -F '\t' '{print $2}' | tr -d '\n')
vfasta=$(grep "^vfasta" $input_file | awk -F '\t' '{print $2}' | tr -d '\n')
dout=$(grep "^dout" $input_file | awk -F '\t' '{print $2}' | tr -d '\n')
...
echo "perl $BASEDIR/detect_variants.pl -ID $sample_name -mode $input_format -T $tumor_input -N $normal_input -R $fasta_ref -O $out_prefix -thread $thread -ram $ram -gatk $gatk -picard $picard -hisat2 $hisat2 -samtools $samtools -gtf $gtf_ref -gene $genelist_ref -dbsnp $dbsnp_ref -hisat2_reference $hisat_ref -germline $germline_ref -pon $PON_ref"
perl $BASEDIR/detect_variants.pl -ID $sample_name -mode $input_format -T $tumor_input -N $normal_input -R $fasta_ref -O $out_prefix -thread $thread -ram $ram -gatk $gatk -picard $picard -hisat2 $hisat2 -samtools $samtools -gtf $gtf_ref -gene $genelist_ref -dbsnp $dbsnp_ref -hisat2_reference $hisat_ref -germline $germline_ref -pon $PON_ref

echo "perl $BASEDIR/filter_variants_apobec3parameters.pl -ID $sample_name -O $out_prefix -R $fasta_ref -fnvcf $fnvcf -fivcf $fivcf -bedf $bedf -bamf $bamf -igg $igg_ref -hla $hla_ref -pseudo $pseudo_ref -tcga $tcga_PON_ref -radar $radar_ref -darned $darned_ref -redi $REDI_ref -summaryOut $summaryOut" 
perl $BASEDIR/filter_variants.pl -ID $sample_name \
-O $out_prefix \
-R $fasta_ref \
-fnvcf $fnvcf \
-fivcf $fivcf \
...
echo "perl $BASEDIR/machine_learning.pl -ID $sample_name -O $out_prefix -gtf $gtf_ref"
perl $BASEDIR/machine_learning.pl -ID $sample_name -O $out_prefix -gtf $gtf_ref -flvcf $flvcf -mout $mout -bout $bout -vfasta $vfasta -dout $dout
